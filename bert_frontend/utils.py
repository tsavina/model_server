...
def predict_bert(source, question, ovms_address, ovms_port):
    # create grpc connection
    channel = grpc.insecure_channel("{}:{}".format(ovms_address, ovms_port))
    ...
    while c_e > c_s:
        ...
        print("Requesting inference on the data chunk...")
        result = stub.Predict(request, 10.0) # result includes a dictionary with all model outputs
        print("Inferece results ready.")
        #print("\nresult:", result)
        res = {}
    ...
    for _, s, e in answers[:3]:
        ...
        answer = context[c_s:s] + "<b>" + context[s:e] + "</b>" + context[e:c_e]
        response = "<br><br>".join([response, answer])
    return response[8:]
